...
def load_and_clean(filepath):
    ...
    # Remove same-month leakage columns
    same_month_leakage = [
        "total_visits", "unique_clients", "unique_households",
        "avg_household_size", "avg_client_age",
    ]

    # Remove unlagged external indicators (keep only _lag1 versions)
    unlagged_external = [
        "ow_cases_official", "ow_beneficiaries",
    ...
    # Redundant columns
    redundant = ["quarter", "unemployment_lag1"]

    cols_to_drop = same_month_leakage + unlagged_external + redundant
    df = df.drop(columns=[c for c in cols_to_drop if c in df.columns])

    # Restrict to fully-populated window
